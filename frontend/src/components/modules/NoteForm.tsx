...
export default function NoteForm({ isEdit = false }) {
  ...
  useEffect(() => {
    const fetchNoteAndCategories = async () => {
      ...
      try {
        ...
        if (!noteData?.data) throw new Error("Note data is empty");

        setTitle(noteData.data.title || "");
        setContent(noteData.data.content || "");
        setCategoryId(noteData.data.category?._id || "");
        if (noteData.data.image) setPreviewImage(noteData.data.image);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.append(
        "data",
        JSON.stringify({ title, content, category: categoryId })
      );
      ...
      if (!res.ok || !data.success)
        throw new Error(data.message || `Failed to ${isEdit ? "update" : "create"} note`);

      toast.success(data.message || `Note ${isEdit ? "updated" : "created"} successfully`);
      router.push("/notes");
      router.refresh();
    } catch (error) {
      const err = error as Error;
  ...
  if (!isLoading && categories.length === 0) {
    return (
      <div className="max-w-2xl mx-auto py-20 text-center">
        <h2 className="text-xl font-semibold text-gray-700 mb-4">No categories found</h2>
        <p className="text-gray-500 mb-6">Please create a category before creating a note.</p>
        <button
